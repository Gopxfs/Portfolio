...
function selectProject(projectNumber) {
  ...
  document.getElementById("live-link").href =
    projects[projectNumber].liveVersion;
  ...
  for (let i = 0; i < projects[projectNumber].technologies.length; i += 1) {
    ...
    listItem.appendChild(
      document.createTextNode(projects[projectNumber].technologies[i])
    );
...
const toggleProjectWindow = document.getElementsByClassName(
...
document.getElementById("contact-form").addEventListener("submit", (event) => {
  ...
  if (email !== email.toLowerCase()) {
    ...
    // If everything is ok, then store it on localStore
    userData.name = name;
